...
void ChatServer::accept_loop() 
{
    while (running_) 
    {
        ...
        utils::send_server_text(client_fd, "INFO Commands: /join <group>, /list, /leave, /audio <file>, /quit.");
        client_threads_.emplace_back([this, client_fd, client_id] {client_read_loop(client_fd, client_id);});
    }
}
...
void ChatServer::client_read_loop(int client_fd, int client_id) 
{
    ...
    while (utils::recv_frame(client_fd, type, payload)) 
    {
        if (type == protocol::FRAME_TEXT) 
        {
            ...
            {
                handle_text_line(client_fd, line);
            }, line.size());
        } 
